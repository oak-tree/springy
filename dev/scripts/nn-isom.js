...
Layout.ISOM.prototype.updateAdaption = function(timestamp){
	return Math.max(this.minAdaption,Math.exp(-1 * this.coolingFactor*(timestamp/this.epoch)) * this.maxAdaption);
}
/** 
 ...
 */
Layout.ISOM.prototype.updatesNodesByDistance = function(w,i,adapation,radius){
	...
	this.eachNode(function(v,point){
		...
		/* check if its far from w by most radius*/
		if (d > radius) {
			return;
		}
		
		/* if so update */
		var update = point.p.subtract(i).multiply(adapation * Math.pow(2,-1 * d));
...
Layout.ISOM.prototype.tick = function(timestep,r) {
	...
	this.updatesNodesByDistance(w,i,adapation,r);
